...
Public Class GameplayForm
    ...
    Sub ResizeForm()
        ...
        Size = New Size(UserOptionsBox.Location.X + UserOptionsBox.Width + 10, Math.Max(UserOptionsBox.Height + 43, BoardBox.Height + 43) + 43)
                            ...
                            If GameCells(Rect.X / 16, Rect.Y / 16).GetState <> "0" Then
                                If GameCells(Rect.X / 16, Rect.Y / 16).GetAdjacents.FindAll(Function(x) x.IsFlagged).Count = Val(GameCells(Rect.X / 16, Rect.Y / 16).GetState) Then
                                    For Each Adjacent In GameCells(Rect.X / 16, Rect.Y / 16).GetAdjacents
                                        If Not Adjacent.IsFlagged And Not Adjacent.IsOpen Then OpenCell(Adjacent.GetLocation)
                                    Next
                                End If
                            End If
                        End If
            ...
            If Ended = "" Then
                ...
                Select Case GameCells(Location.X / 16, Location.Y / 16).GetState
                    Case "0"
                        ...
                        '   If Not GameCells(Location.X / 16, Location.Y / 16).IsOpen Then
                        Open0Cells(g, GameCells(Location.X / 16, Location.Y / 16))
                   ' End If
                    Case "1"
                    ...
                    Case "m"
                        If FirstMove Then ReDim GameCells(GameCells.GetUpperBound(0), GameCells.GetUpperBound(1)) : ResetGame(GameCells.GetUpperBound(0) + 1, GameCells.GetUpperBound(1) + 1) : OpenCell(Location) : Exit Select
