...
Module GamePlayModule
    ...
    Class Game
        ...
        Public UnknownCells As Integer
        Public MineCells As Integer
        ...
        Private Function EvaluateBoard()
            ...
            Do
                ...
                End If
                If SafeCells.Count = 0 Then
                    myConstraintSolver = New ConstraintSolver(Cells, SafeCells, False)
        ...
        Overridable Function GetCells() As Cell(,)
            ...
            UnknownCells = 0
            For Each Cell In Cells
                If Cell.CellContents = "u" Then UnknownCells += 1
    ...
    Class ConstraintSolver ' acts as the second algorithm used to solve the board
        ...
        Sub New(CellArray As Cell(,), SafeCellList As List(Of Cell), Custom As Boolean)
            CustomBoard = Custom
            OldCells = CellArray
        ...
        Public Sub FindNonTrivialSolutions()
            ...
            For Each Cell In Cells
                    ...
                    For Each OtherCell In Cells
                        If Not OtherCell.IsUseless AndAlso Cell IsNot OtherCell AndAlso Not Cell.Adjacents.Count < OtherCell.Adjacents.Count Then ' only if cell has a higher number of adjacents then othercell to avoid negative errors
                            ...
                            If SharedAdjacentCount = OtherCell.Adjacents.Count Then ' if cell contains all the adjacents of other cell then other cell can be substituted into it
                                ...
                                If Cell.CellContents = "-" Then SafeCells = Nothing : Exit Sub 'custom board only, stops if board is invalid
                ...
                End If
            Next
            If SafeCells.Count = 0 And Not CustomBoard Then ' if no safecells are found then move to brute force
                EvaluateeEquations()
                EvaluateEquations()
        ...
        Sub EvaluateeEquations()
            Dim Solver As BruteForceSolver
            For Each Equation In EquationList
                Solver = New BruteForceSolver(Equation, Cells, SafeCells, OldCells, CustomBoard)
                Solver.FindSolutions()
                AllSolutions.Add(Solver.GetSolutions)
            Next
            Dim Count As New List(Of Integer)
            Dim SolutionCombos As New List(Of SolutionCombo)
            For x = 0 To AllSolutions.Count - 1
                Count.Add(0)
            Next
            EnumeratePermutations(Count)
        End Sub
        ...
        Private Sub EvaluateEquations()
            ...
            '                               End Sub)
            For Each Equation In EquationList
                Dim Solver As New BruteForceSolver(Equation, Cells, SafeCells, OldCells, CustomBoard)
                Solver.FindSolutions()
                Solver.DeduceSafeCells()
                For Each SafeCell In Solver.GetSafeCells
                    If Not SafeCells.Contains(SafeCell) Then SafeCells.Add(SafeCell)
    ...
    Class BruteForceSolver
        ...
        Public Sub New(EquationToSolve As CoupledEquation, CellArray(,) As Cell, SafeCellList As List(Of Cell), OriginalCells As Cell(,), Custom As Boolean)
            CustomBoard = Custom
        ...
        Public Sub DeduceSafeCells()
            ...
            If CustomBoard And Solutions.Count = 0 Then Exit Sub
    ...
    Class SolutionCombo
        ...
        Private NumberOfOccurences As Double
        Private ProbabilityOfOccurence As Decimal
        ...
        ReadOnly Property GetNumCells As Integer
            Get
                Dim n As Integer
                For Each Solution In Solutions
                    n += Solution.GetVariables.Count
                Next
                Return n
            End Get
        End Property
