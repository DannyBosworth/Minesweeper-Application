...
Class OverallStatsForm
    ...
    Sub InitiateTables()
        HumanStats = New Stats(File.ReadAllText(Directory.GetCurrentDirectory + "\HumanStats.txt"))
        ComputerStats = New Stats(File.ReadAllText(Directory.GetCurrentDirectory + "\ComputerStats.txt"))
    ...
    Sub ReturnToGame(sender As Object, e As DataGridViewCellEventArgs) Handles StatsTable.CellDoubleClick
        Hide()
        myGameplayForm.GameFromStats(StatsTable.Rows(e.RowIndex).Cells(5).Value)
    End Sub
End Class
...
Class Stats
    ...
    Sub New(Stats As String)
        ...
        For y = Stats.IndexOf(vbLf) + 1 To Stats.Length - 1
            If y = 0 Then Exit For
            If Stats(y) = vbLf Then Continue For
            If Stats(y) <> "," AndAlso Stats(y) <> vbCr Then
                TempStr += Stats(y).ToString
            ElseIf Stats(y) = vbCr Then
                CommaCount = 0
